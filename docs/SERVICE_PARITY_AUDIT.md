...
- **BehaviorService** - Full PostgreSQL + FAISS vector indexThis document maps every GuideAI service to its available interfaces (CLI commands, MCP tools, REST API endpoints). Gaps indicate missing integrations that break the parity contract outlined in `contracts/MCP_SERVER_DESIGN.md`.
...
guideai bci retrieve           ✅ (via CLIReflectionAdapter -> BCIService)**Related**: `contracts/MCP_SERVER_DESIGN.md` (§4-6), `contracts/ACTION_SERVICE_CONTRACT.md`, `PRD.md` (parity requirements)
