...
### Epic E4 — Learning Loop, Analytics, and Governance (GUIDEAI-278) ✅ COMPLETE
...
- ✅ **T4.1.1 Telemetry Schema Definitions:** Typed event payloads (`guideai/telemetry_events.py`), JSON Schemas under `schema/telemetry/v1/`, 8 E4 domain events (pack.activated, pack.deactivated, pack.overlay_applied, bci.retrieval_completed, bci.injection_completed, bci.citation_validated, reflection.candidate_extracted, reflection.candidate_approved).
...
- ✅ **T4.1.3 Telemetry Query API:** CLI `guideai telemetry query/dashboard`, MCP tools `telemetry.query`/`telemetry.dashboard`, parity test suite.
...
- ✅ **T4.4.2 Backward-Compat Migration:** Alembic migration for `feature_flags` table, `StorageBackend` detector (Postgres/SQLite/JSON/Unknown), `PackMigrationService` with bootstrap/rollback, CLI `guideai pack bootstrap|rollback|status`, MCP tools `pack.bootstrap`/`pack.rollback`/`pack.status`. 29 migration tests.
