...
> **Last validated**: 2026-03-10 against local GuideAI dev instance.
...
-If you find a bug/issue with the GuideAI platform itself, check if it’s already tracked on the GuideAI Board. If not, create a new work item with the appropriate labels and metadata.- Follow GWS v1.0 naming conventions (see §2.1 below).

### 2.1 GWS v1.0 — Work Item Naming Standard

All work item titles must follow the GuideAI Work Item Standard (GWS):
...
**Source**: `guideai/agents/work_item_planner/prompts.py` (single source of truth)
...
- Board name: `GuideAI Board`
- Board ID (current): `523b3a4f-4157-4fd1-b5e9-93437eca6009`
- Owner: `nick.sanders.a@gmail.com` (user ID `112316240869466547718`)

> **⚠️ Volatile IDs**: Board IDs, container names, and other infrastructure identifiers **change after every `amprealize fresh` run**. The IDs listed in this document are examples from the last validated run. Before any operation that uses a board ID or container name, agents **must** resolve the current values dynamically:
> - **Board IDs**: Query `board.boards` via `podman exec <db-container> psql ...` or use `GET /api/v1/boards?project_id=<id>`.
> - **Container names**: Run `podman ps --format '{{.Names}}' | grep <service>` to find the current Amprealize-prefixed name.
> - **Project IDs**: These are stable across `amprealize fresh` runs (stored in PostgreSQL). Use `GET /api/v1/projects` or `mcp_guideai_projects_list`.
>
...
All infrastructure is managed through the **Amprealize** standalone package (`packages/amprealize/`). Amprealize provides a Terraform-like workflow (`plan → apply → destroy`) for containerized environments.

> **⚠️ Container names are volatile.** Every `amprealize fresh` run generates a new UUID prefix (e.g., `amp-845682e9-...`). Always discover current names with:
> ```bash
> podman ps --format '{{.Names}}' | sort
> ```

#### Starting the environment
Use Amprealize CLI commands instead of manual `podman start` chains:
...
> **Auto-backup**: Both `nuke` and `fresh` automatically back up all running PostgreSQL databases before destroying anything. Backups are saved to `~/.guideai/backups/` with the tag `pre-nuke`. The last 5 auto-backups are kept. Use `--skip-backup` to opt out.
...
amprealize fresh --force

# Skip the automatic pre-destruction database backup
amprealize fresh --skip-backup
```
...
```bash
podman exec amp-845682e9-f6ad-49a7-80e5-bb29b2155cdf-guideai-db \
...
> **Note**: Amprealize can run migrations automatically on `amprealize apply` when `migrations.auto_run: true` is set in `environments.yaml` (see §5.5).

Refer to `docs/MIGRATION_GUIDE.md` and `behavior_migrate_postgres_schema` for migration procedures.

### 5.5 Amprealize Environment Management

Amprealize is a standalone package (`packages/amprealize/`) that provides blueprint-driven container orchestration for GuideAI infrastructure. It is the **primary tool** for managing the local development stack.

#### Installation
```bash
# Install with CLI support (from repo root)
pip install -e ./packages/amprealize[cli]
...
#### Configuration
Amprealize reads environment definitions from `environments.yaml` at the project root. This file defines:
...
#### Blueprints
Blueprints define the services in each environment. They live in `packages/amprealize/src/amprealize/blueprints/`.
...
- **console**: GuideAI API, gateway (nginx), web-console
...
| Command | Description |
|---|---|
| `amprealize up` | Plan + apply in one step (idempotent; reuses existing env) |
| `amprealize plan <env>` | Preview resource requirements before provisioning |
| `amprealize apply --plan-id <id>` | Provision the planned environment |
| `amprealize status [<run-id>]` | Check status and health of running environment |
| `amprealize restart [--all]` | Restart containers (unhealthy only, or all) |
| `amprealize stop` | Stop containers without removing them |
| `amprealize destroy <run-id>` | Tear down a specific environment |
| `amprealize fresh` | Nuke everything + bring up clean environment |
| `amprealize nuke` | Remove all containers, networks, processes |
| `amprealize list` | List all tracked environments |
| `amprealize blueprints` | List available blueprints |
| `amprealize validate [<path>]` | Validate environments.yaml configuration |
| `amprealize resources` | Show resource usage of running environment |
| `amprealize cleanup` | Clean up stale/orphaned resources |
| `amprealize backup [--tag TAG]` | Back up all running PostgreSQL databases to `~/.guideai/backups/` |
| `amprealize restore [BACKUP_NAME]` | Restore databases from a backup (latest if omitted) |
| `amprealize backups` | List all available database backups |
| `amprealize plan-for-tests` | Plan an environment optimized for test runs |
| `amprealize run-tests` | Run tests in an Amprealize-managed environment |
...
```bash
amprealize restart          # Restart unhealthy containers after reboot
amprealize status           # Check what's running and health status
amprealize resources        # Monitor resource usage
```

**Debugging infra issues:**
```bash
amprealize status           # See which containers are unhealthy
amprealize restart -s guideai-api  # Restart specific service
amprealize fresh            # Nuclear option: rebuild everything
```

**Database backup & restore:**
```bash
amprealize backup                     # Back up all DBs (tag: "manual")
amprealize backup --tag before-migration  # Back up with a custom tag
amprealize backups                    # List all saved backups
amprealize restore                    # Restore from the latest backup
amprealize restore 2026-03-11T10-03-14  # Restore a specific backup
```

**Running tests with Amprealize:**
```bash
./scripts/run_tests.sh --amprealize  # Uses amprealize to manage test infra
amprealize run-tests                  # Or use CLI directly
```

#### Volatile IDs and Dynamic Resolution
Amprealize generates UUID-prefixed container names on each `fresh` or `up --force` run. **Never hardcode container names or board IDs in scripts.** Always resolve dynamically:
...
DB_CONTAINER=$(podman ps --format '{{.Names}}' | grep guideai-db)

# Use amprealize status for structured info
amprealize status --json
```
...
| Tool | Purpose |
|------|---------|
| `mcp_guideai_workitems_create` | Create goals, features, tasks |
...
# Plan and run tests via Amprealize CLI directly
amprealize plan-for-tests
amprealize run-tests
...
1. Check if bug already exists on the GuideAI Board (`board_id: 523b3a4f-4157-4fd1-b5e9-93437eca6009` — resolve dynamically, see §3.1).
2. If not, create a feature (previously called story) with `labels: ["guideai", "platform", "bug"]` and a `metadata.bug_id` field (e.g., `GS4.1`).
...
These are tracked on the GuideAI Board. Agents **must** apply these workarounds until the bugs are fixed.
...
| File | Purpose |
|------|---------|
| `environments.yaml` | Amprealize environment configuration (runtime, blueprints, migrations, variables) |
| `alembic.ini` | Main database migration config |
| `migrations/versions/` | Alembic migration scripts |
| `packages/amprealize/` | Standalone container orchestration package (plan/apply/destroy workflow) |
| `packages/amprealize/src/amprealize/cli.py` | Amprealize CLI (21 commands: up, fresh, nuke, backup, restore, backups, plan, apply, status, etc.) |
| `packages/amprealize/src/amprealize/backup.py` | Database backup/restore via `pg_dump`/`psql` in containers |
| `packages/amprealize/src/amprealize/blueprints/` | Blueprint YAML definitions (local-test-suite, core-data-plane, ci-test-stack, etc.) |
| `packages/amprealize/src/amprealize/service.py` | AmprealizeService core (plan/apply/destroy orchestration) |
| `packages/amprealize/src/amprealize/executors/` | Container runtime executors (PodmanExecutor) |
| `packages/amprealize/src/amprealize/hooks.py` | AmprealizeHooks for ActionService/ComplianceService integration |
...
- `packages/amprealize/README.md` — Amprealize standalone package documentation
- `packages/amprealize/src/amprealize/blueprints/local-test-suite.yaml` — Development blueprint
- `docs/MIGRATION_GUIDE.md` — Database migration procedures
