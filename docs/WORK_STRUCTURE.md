...
>
> **Latest Updates:**
> - ✅ **Workspace Agent Service Extraction (2026-01-16)** - Extracted `AgentWorkspaceManager` into dedicated `workspace-agent` microservice (`packages/_archive/workspace-agent/` — archived 2026-03-30 as part of GUIDEAI-411 production readiness). **Architecture**: Standalone gRPC service (port 50051) exclusively owns podman socket, API server connects via `GuideAIWorkspaceClient`. **Components**: `WorkspaceService` (container lifecycle), `PodmanSocketClient` (socket-based container management), `StateStore` abstraction (InMemory/Redis), gRPC server with bearer token auth (`AuthInterceptor`), protobuf definitions (`proto/workspace/v1/workspace_agent.proto`). **Integration**: Blueprint updated (`local-test-suite.yaml`), call sites migrated (`work_item_execution_service.py`, `tool_executor.py`), old `agent_workspace_manager.py` deprecated. Follows `behavior_extract_standalone_package` pattern (zero guideai core deps, hooks for integration, optional extras).
