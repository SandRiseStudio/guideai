...
| Checklist automation | Engineering | ✅ Completed | `COMPLIANCE_SERVICE_CONTRACT.md`, `guideai/compliance_service.py`, `guideai/adapters.py` (REST/CLI/MCP adapters), `guideai/cli.py`, `tests/test_compliance_service_parity.py` (17 tests passed) — CMD-008 |
| BehaviorService runtime & CLI parity | Engineering + Platform | ✅ Completed | SQLite-backed `guideai/behavior_service.py` (~720 lines), CLI adapters in `guideai/adapters.py`, 9 behaviors subcommands in `guideai/cli.py` (create/list/search/get/update/submit/approve/deprecate/delete-draft), regression coverage in `tests/test_cli_behaviors.py` (2 tests passed), governance updates (`BUILD_TIMELINE.md` #39, `docs/capability_matrix.md`, `PRD.md`, `PRD_NEXT_STEPS.md`) |
| Workflow Engine Foundation | Engineering + DX | ✅ Completed | `WORKFLOW_SERVICE_CONTRACT.md` (service contract with BCI algorithm), SQLite-backed `guideai/workflow_service.py` (~600 lines), CLI adapter + 5 subcommands in `guideai/cli.py` (create-template/list-templates/get-template/run/status), `tests/test_workflow_service.py` (18 passing integration tests), `examples/strategist_workflow_steps.json` (Strategist workflow demo), governance updates (`BUILD_TIMELINE.md` #40, `PROGRESS_TRACKER.md`) |
| VS Code extension MVP | DX + Engineering | ✅ Completed | `extension/package.json`, `extension/tsconfig.json`, `extension/webpack.config.js`, `extension/src/extension.ts`, `extension/src/client/GuideAIClient.ts`, `extension/src/providers/BehaviorTreeDataProvider.ts`, `extension/src/providers/WorkflowTreeDataProvider.ts`, `extension/src/webviews/BehaviorDetailPanel.ts`, `extension/src/webviews/PlanComposerPanel.ts`, `extension/resources/icon.svg`, `extension/resources/workflow.svg`, `extension/.vscode/launch.json`, `extension/.vscode/tasks.json`, `extension/MVP_COMPLETE.md` — 11 TypeScript source files (~1,100 lines), 7 commands, 4 settings, 2 tree views, 2 webview panels, GuideAIClient integration, webpack build (51.1 KiB, 0 vulnerabilities) — `BUILD_TIMELINE.md` #41 |
| VS Code extension runtime validation | DX + Engineering | ✅ Completed | Extension validated in Extension Development Host: fixed activation events (`onStartupFinished`), added workflow icon (`resources/workflow.svg`), implemented JSON format enforcement (`GuideAIClient.withJsonFormat()`), added zero-state messaging (`MessageTreeItem`), fixed workflow tree refresh timing, normalized role matching, seeded sample Strategist behavior. All views functional with live BehaviorService/WorkflowService data; behaviors/workflows load, detail panels render, insertion commands operational, Plan Composer executes workflows. Ready for user testing. — `BUILD_TIMELINE.md` #42 |
| Initial analytics dashboards | Product Analytics | 🚧 In Progress | `docs/analytics/prd_kpi_dashboard_plan.md` (KPI dashboard plan with data requirements, telemetry audit, Snowflake schema, Flink `telemetry-kpi-projector` job, dashboard wireframes); follow-ups: VS Code telemetry emission, execution token accounting enrichment, warehouse schema, Metabase/Looker provisioning |
