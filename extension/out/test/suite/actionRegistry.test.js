...
suite('Action Registry Smoke Tests', () => {
    ...
    test('refreshActionTracker command can be invoked', async () => {
        ...
        try {
            ...
            assert.ok(!message.includes('command \'guideai.refreshActionTracker\' not found'), 'Command should be registered even if execution fails');
        }
    });
    test('openActionTimeline command can be invoked', async () => {
        try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            assert.ok(!message.includes('command \'guideai.openActionTimeline\' not found'), 'Command should be registered even if execution fails');
        }
    });
    test('clearFilters command can be invoked', async () => {
        try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            assert.ok(!message.includes('command \'guideai.actionTracker.clearFilters\' not found'), 'Command should be registered even if execution fails');
        }
    });
    test('Action tracker view container exists', async () => {
        ...
        if (extension) {
            ...
            const hasActionTracker = views.some((v) => v.id === 'guideai.actionTracker');
